...
<template>
  <div class="properties-content">
    <div class="properties-body" v-if="formConfig.currentItem?.itemProps">
      ...
      <Empty class="hint-box" v-if="!formConfig.currentItem.field" description="未选择控件" />
      <Form v-else label-align="left" layout="vertical">
        <div v-for="item of baseFormItemProps" :key="item.name">
          <FormItem :label="item.label" v-if="showProps(item.exclude)">
            <component v-if="item.component" class="component-props" v-bind="item.componentProps" :is="item.component"
        ...
        <FormItem label="是否必选" v-if="!['Grid'].includes(formConfig.currentItem.component)">
          ...
          <Input v-if="formConfig.currentItem.itemProps['required']"
            v-model:value="formConfig.currentItem.itemProps['message']" placeholder="请输入必选提示" />
        </FormItem>
        <FormItem v-if="!['Grid'].includes(formConfig.currentItem.component)" label="校验规则"
