<template>
  <div>
    ...
    <div v-else>
      <div v-for="(item, index) of getOptions[key]" :key="index">
        <div class="options-box">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const getOptions = computed(() => {
        ...
        if (currentItem.componentProps?.customChildrenComponent) {
          //vant 组件的选项在子组件里面
          return currentItem.componentProps.customChildrenComponent.componentProps;
        } else {
          //ant  组件
          return currentItem.componentProps;
        }
      });

      const addOptions = () => {
        if (!getOptions.value?.[key]) getOptions.value![key] = [];
        const len = getOptions.value?.[key].length + 1;

        getOptions.value![key].push({
          label: `选项${len}`,
          value: '' + len,
        });
      };
      const deleteOptions = (index: number) => {
        remove(getOptions.value?.[key], index);
      };
