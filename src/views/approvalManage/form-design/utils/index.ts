...
export function generateKey(formItem?: IVFormComponent): string | boolean {
  try {
    if (formItem && formItem.component) {
      ...
      console.log('[ key ]-28', key, formItem.component)
      formItem.key = key;
      formItem.field = key;
      //设置子组件的field和key
      if (formItem.componentProps && formItem.componentProps.customChildrenComponent) {
        let customChildrenComponent = formItem.componentProps.customChildrenComponent;
        const childrenKey = uniqueId(`${toLine(customChildrenComponent.component)}_`);
        customChildrenComponent.key = childrenKey;
        customChildrenComponent.field = childrenKey;
      }
