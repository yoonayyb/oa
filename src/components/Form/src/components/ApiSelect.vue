...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['options-change', 'change', 'update:value', 'handle-fetch']);
  ...
  async function fetch() {
    ...
    try {
      ...
      loading.value = false;
      // reset status
      // 不知道为什么vben官方会将isFirstLoaded重置为false 这个操作会导致immediate=false状态时 每次点击select都重新请求列表
      // isFirstLoaded.value = false;
    }
  }

  async function handleFetch(visible: boolean) {
    emit('handle-fetch', visible);
    if (visible) {
      if (props.alwaysLoad) {
        await fetch();
