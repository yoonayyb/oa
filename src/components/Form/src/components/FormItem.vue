  ...
  export default defineComponent({
    ...
    setup(props, { slots,attrs }) {
      ...
      const getbordered = computed(() => {
        const { bordered: globbordered } = props.formProps;
        const { bordered:schemabordered } = props.schema;
        const { bordered: itembordered = true } = unref(getComponentsProps);

        let bordered = globbordered || itembordered;
        if (isBoolean(schemabordered)) {
          bordered = schemabordered;
        }
        if (isFunction(schemabordered)) {
          bordered = schemabordered(unref(getValues));
        }
        return bordered;
      });
 ...
 */
 const strToReg = rules => {
  const newRules = cloneDeep(rules);
  return runCode(newRules)
  ...
  try {
    ...
    return code;
      ...
      function renderComponent() {
        ...
        const propsData: Recordable<any> = {//api在这个data里面
          allowClear: true,
          size,
          ...unref(getComponentsProps),
          disabled: unref(getDisable),
          readonly: unref(getReadonly),
          bordered: !unref(getReadonly), //unref(getbordered) 
