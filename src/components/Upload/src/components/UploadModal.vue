<template>
  ...
  >
    ...
    <view class="py-2" v-if="is_netdisk">
      <view class="flex" > 
       <span>文件路径：</span>
      <a-directory-tree
    ...
    <div class="upload-modal-toolbar">
      <Alert :message="getHelpText" type="info" style="word-break: break-all;" banner class="upload-modal-toolbar__text" />
      <Upload
...
<script lang="ts" setup>
  ...
  const emit = defineEmits(['change', 'register', 'delete','reload']);
  ...
  const selectedKeys = ref(props.parent_id) 
  ...
  function beforeUpload(file: File) {
    ...
    } else { //如果是其他文件类型 默认一个图片
      fileListRef.value = [...unref(fileListRef), {...commonItem,thumbUrl:checkFileType(name.split('.').pop()).url}];
    }
    return false;
  }
  ...
  async function uploadApiByItem(item: FileItem) {
    ...
    try {
      ...
      item.file,
      'upload_approve',
      checkFileType(item.type).file_type,
