<template>
  <Dropdown placement="bottomLeft" :overlayClassName="`${prefixCls}-dropdown-overlay`">
    <span :class="[prefixCls, `${prefixCls}--${theme}`]" class="flex">
      <HeadImg :userInfo="getUserInfo" :size="24"> </HeadImg>
      <!-- <img :class="`${prefixCls}__header`" :src="getUserInfo.avatar || headerImg" /> -->
    ...
    <template #overlay>
      <Menu @click="handleMenuClick">
        <MenuItem key="info" :text="'修改个人信息'" icon="ant-design:user-outlined" />
  ...
  <AccountModal :name="'AccountInfoModal'" :type="'account'" @success="handleSuccess" @register="AccountRegister" />
</template>
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const [AccountRegister, AccountUseModal] = useModal();

      function handleLock() {
        openModal(true);
      }
      function handleLnfo() {
        AccountUseModal.openModal(true, {
